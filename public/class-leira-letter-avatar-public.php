...
class Leira_Letter_Avatar_Public{
	...
	public function enqueue_styles() {
		...
		if ( is_admin_bar_showing() && leira_letter_avatar()->is_active() ) {
			$build_url = plugin_dir_url( dirname( __FILE__ ) ) . 'build' . DIRECTORY_SEPARATOR;
			$asset     = include plugin_dir_path( dirname( __FILE__ ) ) . 'build' . DIRECTORY_SEPARATOR . 'public.asset.php';
	 ...
	 * @param  array  $args  Array of arguments to pass to a method.
	...
	public function generate_letter_avatar_url( $id_or_email, $args = array() ) {
		...
		if ( $args['gravatar'] && $this->gravatar_exist( $email_hash ) ) {
			/**
			 * Generate gravatar url and return
			 */
			if ( is_ssl() ) {
				$gravatar = 'https://secure.gravatar.com/avatar/';
			} else {
				$gravatar = sprintf( 'https://%d.gravatar.com/avatar/', wp_rand( 0, 2 ) );
			}
		 ...
		 */
		$bg = $this->get_avatar_background( $letters );

		/**
		 * Determine the letters color now that we have background color
		 */
		$color = $this->get_avatar_color( $bg );

		/**
		 * Font size ratio vs image size
		 */
		$font_ratio = 0.5;
		 ...
		 *  length => character length [1|2] default 2
		 *  name => text to show
	 ...
	 *
	 * @param  array  $data  Array containing all parameters to generate the image
	 *
	 * @return string
	 * @since 1.1.0
	 */
	public function generate_image( $data ) {
		...
		if ( $format != 'svg' ) {
			//Check if the system is able to handle the image
		...
		if ( $save_to_uploads ) {
			...
			if ( ! $avatar_exist ) {
				...
				$avatar_exist = $wp_filesystem->put_contents( $avatar_path, $avatar,
	...
	protected function image_content( $data ) {
		...
		if ( $format != 'svg' ) {
			/**
			 * We're going to create an image 5 times the size requested and in the end resize it to fit the size.
			...
			if ( $rounded ) {
				...
				imagefilledellipse( $image, $circle_size / 2, $circle_size / 2, $circle_size - 2, $circle_size - 2,
					$circle_color );
			} else {
			...
			imagettftext( $image, $font_size, 0, (int) ( ( $big_size - $text_width ) / 2 ),
				(int) ( $font_size + ( ( $big_size - $font_size ) / 2 ) ), $text_color, $font, $text );
		...
		if ( empty( $avatar ) ) {
			$avatar = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="' . $size . 'px" height="' . $size . 'px" viewBox="0 0 ' . $size . ' ' . $size . '" style="user-select: none;"><' . ( $rounded ? 'circle' : 'rect' ) . ' fill="#' . trim( $background,
					'#' ) . '" cx="' . ( $size / 2 ) . '" width="' . $size . '" height="' . $size . '" cy="' . ( $size / 2 ) . '" r="' . ( $size / 2 ) . '"/><text x="50%" y="50%" style="color: #' . trim( $color,
					'#' ) . '; line-height: 1;font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', \'Roboto\', \'Oxygen\', \'Ubuntu\', \'Fira Sans\', \'Droid Sans\', \'Helvetica Neue\', sans-serif;" alignment-baseline="middle" text-anchor="middle" font-size="' . $font_size . '" font-weight="' . ( $bold ? 600 : 400 ) . '" dy=".1em" dominant-baseline="middle" fill="#' . trim( $color,
					'#' ) . '">' . $text . '</text></svg>';
		}
	...
	/**
	 * This method will determine the background color to use
	 * based on the selected method in settings and the letters provided by the user
	 *
	 * @param  string  $letters  The letters to determine the background color
	 *
	 * @return string
	 * @since 1.3.11
	 */
	protected function get_avatar_background( string $letters ): string {
		/**
		...
		switch ( $current_option ) {
			...
			case 'random':
				...
				$key   = mb_strtoupper( trim( $letters ) );// Normalize text
				$hash  = md5( $key );                      // Hash into a number
				$num   = hexdec( substr( $hash, 0, 8 ) );  // Convert the first 8 chars of hash to integer
				$index = $num % count( $backgrounds );     // Pick a color index deterministically

				$bg = $backgrounds[ $index ];
				break;
			case 'auto':
			default:
				//Let's create a unique background color based on the letters
				$string = mb_strtoupper( trim( $letters ) );// Normalize
				$hash   = md5( $string );                   // Hash the string

				// Convert part of the hash into R, G, B
				$r = hexdec( substr( $hash, 0, 2 ) );
				$g = hexdec( substr( $hash, 2, 2 ) );
				$b = hexdec( substr( $hash, 4, 2 ) );

				// Clamp brightness to avoid extremes
				$min = 80;                                  // min channel value (avoid too dark)
				$max = 200;                                 // max channel value (avoid too light)
	...
	/**
	 * Determine the letters color to use based on the background color and selected method settings
	 *
